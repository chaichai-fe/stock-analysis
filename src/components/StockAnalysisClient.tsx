...
export function StockAnalysisClient() {
  ...
  const simulation = useMemo(() => {
    ...
    const holdingYears =
      (endDateMs - entryDateMs) / (365.25 * 24 * 60 * 60 * 1000)
    const cagrPercent =
            ...
            <>
              <div className="border-b border-stone-200 pb-4 dark:border-stone-700">
                ...
                <p className="mt-1 text-sm text-stone-500 dark:text-stone-400">
                  {result.startDate} ～ {result.endDate}（约 {result.years}{' '}
                  年）
                </p>
              </div>
                      ...
                      <div className="mt-4 space-y-2 rounded-lg bg-stone-100 p-4 dark:bg-stone-800">
                        <p className="text-sm text-stone-600 dark:text-stone-400">
                          ...
                          <span className="font-medium text-stone-900 dark:text-stone-100">
                            {simulation.endValue.toLocaleString('zh-CN', {
                        ...
                        {!reinvest && simulation.totalCashReceived > 0 && (
                          <p className="text-sm text-stone-600 dark:text-stone-400">
                            累计分红：
                            <span className="font-medium text-stone-900 dark:text-stone-100">
                              {simulation.totalCashReceived.toLocaleString(
                                'zh-CN',
                                {
                                  minimumFractionDigits: 2,
                                  maximumFractionDigits: 2,
                        ...
                        <p className="text-sm text-stone-600 dark:text-stone-400">
                          总收益：
                          <span
                            className={`font-medium ${simulation.totalReturn >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}
                          >
                            {simulation.totalReturn.toLocaleString('zh-CN', {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}{' '}
                            元
                          ...
                          {simulation.totalReturnPercent.toFixed(2)}%）
                        </p>
                        <p className="text-sm text-stone-600 dark:text-stone-400">
                          年化收益率：
                          <span
                            className={`font-medium ${simulation.cagrPercent >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}
                          >
                            {simulation.cagrPercent >= 0 ? '+' : ''}
                            {simulation.cagrPercent.toFixed(2)}%
                          </span>
                        </p>
                      </div>
